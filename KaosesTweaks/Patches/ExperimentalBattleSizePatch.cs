...
namespace KaosesTweaks.Patches
{
    ...
    public class BattleSizePatchEx_GetRealBattleSize
    {
        static void Postfix(ref int __result)
        {
            ...
            {
                if (BattleSizePatchEx_PartyGroupTroopSupplier.mountfootratio * settings.BattleSizeEx * (1f + (settings.ReinforcementQuota * settings.SlotsForReinforcements * 0.01f)) > 2048)
                {
                    __result = (int)((2048 / BattleSizePatchEx_PartyGroupTroopSupplier.mountfootratio) / (1f + (settings.ReinforcementQuota * settings.SlotsForReinforcements * 0.01f)));
                    ...
                    if (settings.BattleSizeDebug)
                    {
                        ...
                        int SlotsForReinforcements = (int)((2048 - SlotsForMounts) - (2048 - SlotsForMounts) / (1f + (settings.ReinforcementQuota * settings.SlotsForReinforcements * 0.01f)));
                        IM.ColorRedMessage("MountedRatio: " + BattleSizePatchEx_PartyGroupTroopSupplier.mountfootratio + " | Reserved: " + (1f + (settings.ReinforcementQuota * settings.SlotsForReinforcements * 0.01f)));
    ...
    internal class BattleSizePatchEx_BattleSizeSpawnTick
    {
            ...
            if (MCMSettings.Instance is { } settings)
            {
                ...
                foreach (Agent agent in __instance.Mission.AllAgents)
                {
                    if (agent.IsMount)
                    {
                        ...
                        {
                            mountNoRider += 1;
                            if (mountNoRider > settings.RetreatHorses) agent.Retreat(__instance.Mission.GetClosestFleePositionForAgent(agent)); //Mounts retreat more, freeing up agents
                        }
                    }
        ...
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            if (MCMSettings.Instance is { } settings && list.Count == 250)
            {
                ...
                list[83].operand = settings.ReinforcementQuota * 0.01f; //>= (float)this._battleSize * _ReinforcementQuota * 0.01f_ --> Percentage of Battlesize each reinforcement
