...
namespace KaosesTweaks.Tweaks
{
    ...
    class PrisonerImprisonmentTweak
    {
        ...
        {
            if (escapeBehaviour == null || !(Factory.Settings is { } settings) || !hero.IsPrisoner) return;
        ...
        public static void DailyTick()
        {
            foreach (Hero hero in Hero.AllAliveHeroes)
            {
                ...
                if (hero.PartyBelongedToAsPrisoner == null && hero.IsPrisoner && hero.IsAlive && !hero.IsActive && !hero.IsNotSpawned && !hero.IsReleased)
                {
                    ...
                    if (Factory.Settings is { } settings && (days > (settings.MinimumDaysOfImprisonment + 3)))
                    {
                        IM.MessageGreen("Releasing " + hero.Name + " due to Missing Hero Bug. (" + (int)days + " days)");
