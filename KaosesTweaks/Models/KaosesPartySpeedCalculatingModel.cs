...
namespace KaosesPartySpeeds.Model
{
    class KaosesPartySpeedCalculatingModel : DefaultPartySpeedCalculatingModel
    {
        ...
        public override float MinimumSpeed => Statics._settings.KaosesmininumSpeedAmount;
          ...
          int additionalTroopOnHorseCount = 0)
        {
            ...
            KaosesPartySpeedCalculatingModel.AddCargoStats(mobileParty, ref numberOfAvailableMounts, ref totalWeightCarried, ref herdSize1);
            ...
            if (mobileParty.AttachedParties.Count != 0)
            {
                ...
                {
                    KaosesPartySpeedCalculatingModel.AddCargoStats(attachedParty, ref numberOfAvailableMounts, ref totalWeightCarried, ref herdSize1);
            ...
            result.AddFactor(footmenRatioModifier, KaosesPartySpeedCalculatingModel._textMountedFootmen);
            ...
            if (!mobileParty.IsCaravan)
            {
                if (mobileParty.Party.NumberOfPrisoners > mobileParty.Party.PrisonerSizeLimit)
                {
                    ...
                    result.AddFactor(prisonerSizeEffect, KaosesPartySpeedCalculatingModel._textOverPrisonerSize);
                }
                float modifierPrisoner = KaosesPartySpeedCalculatingModel.GetSizeModifierPrisoner(num1, totalManCount);
                result.AddFactor((float)(1.0 / (double)modifierPrisoner - 1.0), KaosesPartySpeedCalculatingModel._textPrisoners);
            }
            if ((double)morale > 70.0)
                result.AddFactor((float)(0.0500000007450581 * (((double)morale - 70.0) / 30.0)), KaosesPartySpeedCalculatingModel._textHighMorale);
            if ((double)morale < 30.0)
                result.AddFactor((float)(-0.100000001490116 * (1.0 - (double)mobileParty.Morale / 30.0)), KaosesPartySpeedCalculatingModel._textLowMorale);
            ...
            if (mobileParty.IsCaravan)
                result.AddFactor(0.1f, KaosesPartySpeedCalculatingModel._textCaravan);
            ...
            switch (faceTerrainType)
            {
                ...
                case TerrainType.Forest:
                    ...
                    if (PartyBaseHelper.HasFeat(mobileParty.Party, DefaultCulturalFeats.BattanianForestSpeedFeat))
                    {
                        ...
                        finalSpeed.AddFactor(num3, KaosesPartySpeedCalculatingModel._culture);
                        break;
                    }
                    break;
            }
            if (Campaign.Current.Models.MapWeatherModel.GetIsSnowTerrainInPos(mobileParty.Position2D.ToVec3()))
                finalSpeed.AddFactor(-0.1f, KaosesPartySpeedCalculatingModel._snow);
            if (Campaign.Current.IsNight)
            {
                finalSpeed.AddFactor(-0.25f, KaosesPartySpeedCalculatingModel._night);
          ...
          ref ExplainedNumber result)
        {
            ...
            result.AddFactor(num, KaosesPartySpeedCalculatingModel._textCavalry);
        }
