...
namespace KaosesTweaks.Models
{
    ...
    public class CharacterDevelopmentModel : DefaultCharacterDevelopmentModel
    {
        ...
        public override int LevelsPerAttributePoint
        {
            get
            {
                if (Factory.Settings.CharacterLevelsPerAttributeModifiers)
                {
                    return Factory.Settings.CharacterLevelsPerAttributeValue;
                }
                return 4;
            }
        }
        ...
        public override int FocusPointsPerLevel
        {
            get
            {
                if (Factory.Settings.CharacterFocusPerLevelModifiers)
                {
                    return Factory.Settings.CharacterFocusPerLevelValue;
                }
                return 1;
            }
        }
        ...
        public override ExplainedNumber CalculateLearningLimit(int attributeValue, int focusValue, TextObject attributeName, bool includeDescriptions = false)
        {
            ...
            if (Factory.Settings.LearningLimitEnabled)
            {
                result.Add(attributeValue * 10 * Factory.Settings.LearningLimitMultiplier, attributeName, null);
                result.Add(focusValue * 30 * Factory.Settings.LearningLimitMultiplier, _skillFocusText, null);
            }
            else
            {
